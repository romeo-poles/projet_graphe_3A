...
public class Path {
        ...
        for (int i = 0; i < nodes.size() - 1; ++i) {
            ...
            for (Arc arc: current.getSuccessors()) {
                if (arc.getDestination().equals(next)) {
                    double travelTime = arc.getMinimumTravelTime();
            ...
            if (bestArc == null) {
                throw new IllegalArgumentException(
                        "Cannot build path from unconnected consecutive nodes.");
            }
            arcs.add(bestArc);
        }
    ...
    public static Path createShortestPathFromNodes(Graph graph, List<Node> nodes)
        ...
        for (int i = 0; i < nodes.size() - 1; ++i) {
            ...
            for (Arc arc: current.getSuccessors()) {
                if (arc.getDestination().equals(next) && arc.getLength() < bestLength) {
                    bestLength = arc.getLength();
                    bestArc = arc;
                }
            }
